...
- 存储 CS 基址的单元为 TYPE * 4 + 2, 存储 IP 偏移量的单元为 TYPE * 4
- 其中 8259 拥有 IRQ0 至 IRQ7 八个中断请求信号（输入信号）, ICW2 设定的就是 IRQ0 所映射的中断向量号, 此后的 IRQ1 至 IRQ7 所映射的中断向量号依次类推, 自动连续映射
- 实例解释: 此代码中针对 ICW2 的设置为 `MOV AL, 08H : OUT 21H, AL`, 此时 IRQ0 所映射的中断向量号为 8, 所以类推可得 IRQ7 所映射的中断向量号为 15, 故计算可得存储 IP 的单元为 `15*4(003CH)`, 存储 CS 的单元为 `15*4+2(003EH)` 
- **中断向量表的起始地址**: x86 架构中的一种默认规定, 中断向量表(Interrupt Vector Table, IVT)在内存中的固定位置开始(0000H), 每个中断向量占用 4 个字节
