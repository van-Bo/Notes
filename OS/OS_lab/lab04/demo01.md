...
## 测试结果展示
<img src="./images/sender-receiver.png" style="width:50%;">
...
## 测试结果展示
<img src="./images/writer-reader.png" style="width:50%;">

---

# 测试代码3——(比较上述两种进程间通信机制下的数据传输时间)
...
## 测试结果展示
<img src="./images/sender-receiver-timed.png" style="width:50%;">
<img src="./images/writer-reader-timed.png" style="width:50%;">
...
* **共享内存**:
    ...
    * **数据传输阶段**：一旦共享内存附接完成，进程对共享内存的读写操作**不直接涉及内核**（除非发生缺页中断等底层内存管理事件）。数据交换直接在用户空间的映射区域进行，速度非常快。
...
* **共享内存 (实验测试中使用标志位同步)**:
    ...
    * **忙等待非常消耗CPU周期**。当一个进程在等待另一个进程设置标志位时，它会持续执行循环检查，浪费CPU资源。这部分CPU消耗会计入到我们测量的时间中，特别是如果进程切换不频繁，忙等的时间会很长。
